...
struct StatsView: View {
    ...
    private var countries: [Country] {
        Array(Set(dataManager.wines.compactMap(\.country).filter { $0 != .france }))
            .sorted { $0.rawValue < $1.rawValue }
    }

    private var regions: [Region] {
        Array(Set(dataManager.wines.filter { $0.country == .france }.compactMap(\.region)))
            .sorted { $0.rawValue < $1.rawValue }
    }

    private var types: [WineType] {
        Array(Set(dataManager.wines.compactMap(\.type)))
            .sorted { $0.rawValue < $1.rawValue }
    }

    private var years: [Int] {
        Array(Set(dataManager.quantities.compactMap(\.year))).sorted(by: >)
    }
        ...
        NavigationStack {
            VStack(spacing: CharterConstants.margin) {
                VStack(spacing: CharterConstants.marginSmall) {
                    ...
                    tile(left: String(localized: "Montant total"),
                         right: "\(generalInfos.price.toRoundedString) \(String(describing: Locale.current.currencySymbol ?? "€"))")
                }
                ...
                NeoTabsView(items: [
                    NeoTabsItemModel(title: String(localized: "Régions"),
                                     index: 0) {
                        AnyView(ChartsView(data: regions.map { StepCount(name: $0.description, count: quantity(for: $0)) } + countries
                                .map { StepCount(name: $0.description,
