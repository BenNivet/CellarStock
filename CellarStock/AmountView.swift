...
struct AmountView: View {
    ...
    init(year: Binding<Int>, pricesByYear: Binding<[Int: Double]>) {
        ...
        _amount = if amountInt == 0 {
            State(wrappedValue: "")
        } else {
            State(wrappedValue: String(amountInt))
        }
    }
    ...
    var body: some View {
        NavigationView {
            VStack {
                FloatingTextField(placeHolder: String(localized: "Prix"), text: $amount,
                                  rightIconString: String(describing: Locale.current.currencySymbol ?? "€"))
